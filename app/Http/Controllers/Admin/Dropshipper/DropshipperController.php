...
class DropshipperController extends BaseController
{
    ...
    {
        $vendor = $this->supplierRepo->getFirstWhere(params: ['id' => $request['id']]);
        $this->supplierRepo->update(id: $request['id'], data: ['status' => $request['status']]);
        if ($request['status'] == "approved") {
            ...
            $this->supplierRepo->update(id: $request['id'], data: ['auth_token' => Str::random(80)]);
    ...
    {
        $vendors = $this->supplierRepo->getListWhere(
            orderBy: ['id' => 'desc'],
            searchValue: $request['searchValue'],
            relations: ['orders', 'product'],
            dataLimit: 'all'
    ...
    public function updateSalesCommission(Request $request, $id): RedirectResponse
    {
        ...
        $this->supplierRepo->update(id: $id, data: ['sales_commission_percentage' => $request['commission_status'] == 1 ? $request['commission'] : null]);
    ...
    public function getView(Request $request, $id, $tab = null): View|RedirectResponse
    {
        ...
        $seller = $this->supplierRepo->getFirstWhere(
    ...
    public function updateSetting(Request $request, $id): RedirectResponse
    {
        ...
        if ($request->has('gst')) {
            if ($request['gst_status'] == 1 && $request['gst'] == null) {
                ...
                $this->supplierRepo->update(id: $id, data: ['gst' => $request['gst_status'] == 1 ? $request['gst'] : null]);
                Toastr::success(translate('GST_number_for_this_vendor_has_been_updated.'));
            }
        }
        if ($request->has('seller_pos_update')) {
            $this->supplierRepo->update(id: $id, data: ['pos_status' => $request->get('seller_pos', 0)]);
