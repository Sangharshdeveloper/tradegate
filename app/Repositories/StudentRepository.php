...
class StudentRepository implements SupplierRepositoryInterface
{
    ...
    {
        return $this->supplier->with($relations)->whereNotIn('status', [$status])->paginate($paginateBy);
    }
    ...
    {
        return $this->supplier->with($relations)
    ...
    {
        $query = $this->supplier->with($relations)->when(!empty($orderBy), function ($query) use ($orderBy) {
    ...
    {
        $this->supplier->where($params)->delete();
        return true;
    }
}
