    ...
    Route::group(['middleware' => ['module:system_settings']], function () {
     ...
     Route::group(['prefix' => 'suppliers', 'as' => 'suppliers.', 'middleware' => ['module:user_section']], function () {
        Route::controller(SupplierController::class)->group(function () {
            ...
            Route::get(Supplier::ORDER_LIST_EXPORT[URI] . '/{supplier_id}', 'exportOrderList')->name('order-list-export');
            Route::post(Supplier::STATUS[URI], 'updateStatus')->name('updateStatus');
            Route::get(Supplier::EXPORT[URI], 'exportList')->name('export');
            Route::get(Supplier::PRODUCT_LIST[URI] . '/{supplier_id}', 'getProductListView')->name('product-list');

            Route::post(Supplier::SALES_COMMISSION_UPDATE[URI] . '/{id}', 'updateSalesCommission')->name('sales-commission-update');
            Route::get(Supplier::ORDER_DETAILS[URI] . '/{order_id}/{supplier_id}', 'getOrderDetailsView')->name('order-details');
            Route::get(Supplier::VIEW[URI] . '/{id}/{tab?}', 'getView')->name('view');
            Route::post(Supplier::UPDATE_SETTING[URI] . '/{id}', 'updateSetting')->name('update-setting');

            Route::get(Supplier::WITHDRAW_LIST[URI], 'getWithdrawListView')->name('withdraw_list');
            Route::get(Supplier::WITHDRAW_LIST_EXPORT[URI], 'exportWithdrawList')->name('withdraw-list-export-excel');
            Route::get(Supplier::WITHDRAW_VIEW[URI] . '/{withdrawId}/{vendorId}', 'getWithdrawView')->name('withdraw_view');
            Route::post(Supplier::WITHDRAW_STATUS[URI] . '/{id}', 'withdrawStatus')->name('withdraw_status');
        });
    });    
